...
module.exports = async (
  ...
  const agents = systemPrompts.map((systemPrompt, i) => {
    return new Agent(agentName[i], `agent-${i}-${conversationId}`, {
      systemPrompt,
    });
  });
  ...
  const allVoiceIds = [
    ...
    "zXCRxA2YIebDYIth1e99", // Rafinha Bastos em ingles
    "yLD9slUCiPhajm685IDj", // Trevor Noah
  ];

  if (createAudio) {
    const voiceIds = [allVoiceIds[3], allVoiceIds[2]];

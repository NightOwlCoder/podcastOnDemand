...
exports.synthesizeTextEleven = async (text, { voiceId = 'yoZ06aMxZJJ28mfd3POQ' } = {}) => {
    ...
    if (!response.ok) {
        console.error(`ERROR: API call to ${ttsUrl} failed with status ${response.status} ${response.statusText}`);
...
exports.concatenateAudioFiles = async (audioFiles, conversationId) => {
    ...
    // try {
    //     await fs.unlink(`${conversationId}.mp3`);
    //   } catch (err) {
    //     // ignored
    //   }
    ...
    try {
        await new Promise((resolve, reject) => {
            exec(
                `ffmpeg -y -f concat -safe 0 -i ${audioListFilePath} -c copy ${conversationId}.mp3`,
                (error, stdout, stderr) => {
